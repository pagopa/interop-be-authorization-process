package it.pagopa.pdnd.interop.uservice.authorizationprocess.server

import akka.http.scaladsl.Http
import akka.http.scaladsl.server.Route
import it.pagopa.pdnd.interop.uservice.authorizationprocess.api.AuthApi
import it.pagopa.pdnd.interop.uservice.authorizationprocess.api.ClientApi
import it.pagopa.pdnd.interop.uservice.authorizationprocess.api.OperatorApi
import it.pagopa.pdnd.interop.uservice.authorizationprocess.api.WellKnownApi
...
class Controller(auth: AuthApi, client: ClientApi, operator: OperatorApi, wellKnown: WellKnownApi, validationExceptionToRoute: Option[ValidationException => Route] = None)(implicit system: ActorSystem) {
  ...
  */
  def getBuildInfo: Route =
    path("pdnd-interop-uservice-authorization-process" / "build-info") {
      get {
        complete(HttpEntity(ContentTypes.`application/json`, buildinfo.BuildInfo.toJson))
      }
   }

  lazy val routes: Route = getBuildInfo ~ pathPrefix("pdnd-interop-uservice-authorization-process" / interfaceVersion) {
    toStrictEntity(strictnessTimeout) {
      extractRequest {
        request =>
          validationFunction(request){
            auth.route  ~ client.route  ~ operator.route  ~ wellKnown.route 
        }
